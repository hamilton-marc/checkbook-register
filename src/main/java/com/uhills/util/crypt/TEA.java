...
// another key
        byte key[] = new java.math.BigInteger("67352FEFD4D43DB520AFE5D80BC7CCB", 16).toByteArray();
        ...
        System.out.println("Hex string length: " + hexStr.length());

        byte dec[] = t.decode(enc);
        System.out.println("\nDecoded output = " + new String(dec));

        enc = t.fromHex(hexStr);
        dec = t.decode(enc);
        System.out.println("\nDecoded hex string = " + new String(dec));
    }
...
//      int counter = enc.length / 2;
    ...
    */
    ...
    public int[] fromHex(String strHex) throws ArrayIndexOutOfBoundsException, NumberFormatException
    {
        if ((strHex.length() % 8) > 0)
            throw new ArrayIndexOutOfBoundsException("Odd number of bytes found: " + strHex.length());

        int[]       encoding = new int[strHex.length() / 8];
//      byte[]      buffer = new byte[8];
        String[]    strHexBuf = new String[encoding.length];

        for (int i=0; i < encoding.length; i++)
        {
            strHexBuf[i] = new String(strHex.substring(i * 8, i * 8 + 8)).trim();

            // we have to use a Long because the morons at Sun
            // didn't take into account the fact that the hex
            // string might be larger than 0x7fffffff.
            encoding[i] = (int) Long.parseLong(strHexBuf[i], 16);
        }
